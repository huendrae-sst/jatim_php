            ...
            <table class="table table-hover table-striped align-middle mb-0 text-nowrap fs-8">
                ...
                <tbody>
                    @forelse($allocatedOrders as $ord)
                        @php
                            $totalAllocated = $ord->items->sum('qty_allocated');
                            ...
                            $ordData = [
                                ...
                                'items' => $ord->items->map(fn($it) => [
                                    ...
                                    'qty_allocated' => $it->qty_allocated,
                                ])->values()->all(),
                        ...
                        <tr>
                            ...
                            <td>
                                @if($itemsCount > 1)
                                    ...
                                    <div class="fs-9 text-secondary text-truncate" style="max-width: 280px;" title="{{ $ord->items->map(fn($i) => ($i->item->name ?? '-') . ' (' . $i->qty_allocated . ' ' . ($i->item->uom ?? '') . ')')->implode(', ') }}">
                                        {{ $ord->items->take(2)->map(fn($i) => ($i->item->name ?? '-') . ' (' . $i->qty_allocated . ' ' . ($i->item->uom ?? '') . ')')->implode(', ') }}{{ $itemsCount > 2 ? ', +' . ($itemsCount - 2) . ' lainnya' : '' }}
                                    </div>
                                @elseif($firstItem)
                                    <div class="fw-bold text-body">{{ $firstItem->name }}</div>
                                    <div class="fs-9 text-secondary font-monospace">{{ $firstItem->sku }} • {{ $ord->items->first()->qty_allocated }} {{ $firstItem->uom }}</div>
                                @else
